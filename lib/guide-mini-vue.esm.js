...
function finishComponentSetup(instance) {
    ...
    if (compiler && !Component.render) {
        if (Component.template) {
            console.log(123, compiler(Component.template));
...
function transformExpression(node) {
    if (node.type === 0 /* NodeTypes.INTERPOLATION */) {
        return () => node.content = processExpression(node.content);
    }
}
...
export { createApp, createVNode as createElementVNode, createRender, createTextVNode, getCurrentInstance, h, inject, nextTick, provide, ref, renderSlots, toDisplayString };
